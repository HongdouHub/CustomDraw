...
public class ViewWatcher {
    ...
    private int mWidth, mHeight;                    // Item的宽高
    private int mControlWidth, mControlHeight;      // 控制点的宽高

    private float[] srcPosition, dstPosition;
    private RectF srcRect, dstRect;                 // 背景点
    private Point prePivot, lastPivot;              // 平移时的参考点（中心点坐标）
    ...
    private boolean enable;                         // 是否监听平移、缩放和旋转
    private boolean drawRectEnable;                 // 是否绘制背景,以便测试矩形的映射
    private boolean drawFrameEnable;                // 是否绘制边框,以便测试点的映射
    private boolean drawControllerEnable;           // 是否绘制控制点图片

    private Paint mRectPaint;                       // 背景画笔
    ...
    public ViewWatcher(Builder builder) {
        this.enable = builder.enable;
        this.mControlWidth = builder.mControlWidth;
        this.mControlHeight = builder.mControlHeight;
        this.mWidth = builder.mWidth;
        this.mHeight = builder.mHeight;
        this.matrix = builder.matrix;
        this.drawRectEnable = builder.drawRectEnable;
        this.drawFrameEnable = builder.drawFrameEnable;
        this.drawControllerEnable = builder.drawControllerEnable;
    ...
    public boolean watchOnTouchEvent(MotionEvent event) {
        if (!enable) {
            return false;
        }
        ...
        Log.i(DRAW_VIEW_TAG, "watchOnTouchEvent operateType: " + operateType);
    ...
    // 绘制控制点图片
    public void drawController(Canvas canvas, Bitmap controlBitmap, Paint paint) {
        if (drawControllerEnable) {
            for (int i = 0; i < dstPosition.length; i += 2) {
                canvas.drawBitmap(controlBitmap,
    ...
    public static final class Builder {
        ...
        private boolean drawRectEnable;
        private boolean drawFrameEnable;
        private boolean drawControllerEnable;

        public Builder setWatchEnable(boolean enable) {
            this.enable = enable;
            return this;
        }
        ...
        public Builder setControlSize(int width, int height) {
        ...
        public Builder setDrawRectEnable(boolean drawRectEnable) {
            this.drawRectEnable = drawRectEnable;
            return this;
        }

        public Builder setDrawFrameEnable(boolean drawFrameEnable) {
            this.drawFrameEnable = drawFrameEnable;
            return this;
        }

        public Builder setDrawControllerEnable(boolean drawControllerEnable) {
            this.drawControllerEnable = drawControllerEnable;
            return this;
        }

        public ViewWatcher build() {
            return new ViewWatcher(this);
        }
    }
}
