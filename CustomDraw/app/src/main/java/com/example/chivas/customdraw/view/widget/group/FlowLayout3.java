...
//                int xVelocity = (int) velocityTracker.getXVelocity();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            MyFlowLayoutParams lp = (MyFlowLayoutParams) childView.getLayoutParams();
            marginLeft = Math.max(0, lp.leftMargin);
            marginRight = Math.max(0, lp.rightMargin);
            marginTop = Math.max(0, lp.topMargin);
            marginBottom = Math.max(0, lp.bottomMargin);

            // 5. 获取到当前子View的测量的宽度/高度
            int childWight = childView.getMeasuredWidth() + marginLeft + marginRight;
            int childHeight = childView.getMeasuredHeight() + marginTop + marginBottom;
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < lineCount; i++) {
            ...
            for (int j = 0; j < size; j++) {
                ...
                // 确定下一个View的left
                currX += childView.getMeasuredWidth() + lp.leftMargin + lp.rightMargin + getPaddingLeft();
            }
